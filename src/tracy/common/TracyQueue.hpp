...
#pragma pack(push, 1)
	...
	enum class LockType : uint8_t { Lockable, SharedLockable };
	...
	enum class GpuContextType : uint8_t { Invalid, OpenGl, Vulkan, OpenCL, Direct3D12, Direct3D11 };

	enum GpuContextFlags : uint8_t { GpuContextCalibration = 1 << 0 };
