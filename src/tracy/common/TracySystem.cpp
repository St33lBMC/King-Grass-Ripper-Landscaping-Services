...
namespace tracy {
...
#if defined _WIN32
			static_assert(
				sizeof(decltype(GetCurrentThreadId())) <= sizeof(uint32_t),
	...
	TRACY_API void SetThreadNameWithHint(const char* name, int32_t groupHint) {
...
#ifdef TRACY_ENABLE
		{
			...
			while (!GetThreadNameData()
						.compare_exchange_weak(data->next, data, std::memory_order_release, std::memory_order_relaxed)
			) {}
		}
#endif
	}
