...
namespace tracy {
	...
	enum class EventPhase : uint8_t { Begin, End };
	...
	class OpenCLCtx {
		public:
			...
			OpenCLCtx(cl_context context, cl_device_id device) :
				...
				TRACY_CL_CHECK_ERROR(clEnqueueWriteBuffer(
					queue,
					dummyBuffer,
					CL_FALSE,
					0,
				...
				cl_int eventStatus;
				TRACY_CL_CHECK_ERROR(clGetEventInfo(
					writeBufferEvent,
				...
				TRACY_CL_CHECK_ERROR(clGetEventProfilingInfo(
					writeBufferEvent,
			...
			void Collect() {
				...
				for (; m_tail != m_head; m_tail = (m_tail + 1) % QueryCount) {
					...
					if (err != CL_SUCCESS) {
						...
						assert(
							false && "clGetEventInfo failed, maybe a TracyCLZone is not paired with TracyCLZoneSetEvent"
						);
						continue;
					}
					...
					if (err == CL_PROFILING_INFO_NOT_AVAILABLE) {
						TracyMessageL(
							"command queue is not created with CL_QUEUE_PROFILING_ENABLE flag, check your code!"
						);
	...
	class OpenCLCtxScope {
		public:
			...
			tracy_force_inline
			OpenCLCtxScope(OpenCLCtx* ctx, const SourceLocationData* srcLoc, int depth, bool is_active)
	#ifdef TRACY_ON_DEMAND
				:
				m_active(is_active && GetProfiler().IsConnected())
	#else
...
using TracyCLCtx = tracy::OpenCLCtx*;
	...
	#define TracyCLContextName(ctx, name, size) ctx->Name(name, size);
		...
		#define TracyCLNamedZone(ctx, varname, name, active)                                                           \
			...
			tracy::OpenCLCtxScope                                                                                      \
				varname(ctx, &TracyConcat(__tracy_gpu_source_location, TracyLine), TRACY_CALLSTACK, active);
