...
namespace tracy {
	...
	namespace detail {
		static inline int noop(lua_State* L) {
			return 0;
		}
	} // namespace detail
	...
	namespace detail {
		...
		static tracy_force_inline void SendLuaCallstack(lua_State* L, uint32_t depth) {
			...
			for (cnt = 0; cnt < depth; cnt++) {
				if (lua_getstack(L, cnt + 1, dbg + cnt) == 0)
					break;
		...
		static inline int LuaZoneBeginS(lua_State* L) {
			...
			if (zoneCnt != 0 && !GetLuaZoneState().active)
				return 0;
			...
			const auto srcloc =
				Profiler::AllocSourceLocation(dbg.currentline, src, dbg.name ? dbg.name : dbg.short_src);
