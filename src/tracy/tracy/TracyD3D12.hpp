...
namespace tracy {
	...
	class D3D12QueueCtx {
		...
		public:
			D3D12QueueCtx(ID3D12Device* device, ID3D12CommandQueue* queue) : m_device(device), m_queue(queue) {
				...
				static constexpr uint32_t MaxQueries =
					64 * 1024; // Must be even, because queries are (begin, end) pairs
				...
				MemWrite(
					&item->gpuNewContext.thread,
					decltype(item->gpuNewContext.thread)(0)
				); // #TODO: why 0 instead of GetThreadHandle()?
	...
	class D3D12ZoneScope {
		...
		public:
			tracy_force_inline D3D12ZoneScope(
				...
				const SourceLocationData* srcLocation,
				bool active
			) :
			...
			tracy_force_inline ~D3D12ZoneScope() {
				...
				m_cmdList->ResolveQueryData(
					...
					m_queryId,
					2,
					m_ctx->m_readbackBuffer,
					m_queryId * sizeof(uint64_t)
				);
			}
	};
...
using TracyD3D12Ctx = tracy::D3D12QueueCtx*;
	...
	#define TracyD3D12SrcLocObject(name, color)                                                                        \
		static constexpr tracy::SourceLocationData                                                                     \
			TracyD3D12SrcLocSymbol {name, TracyFunction, TracyFile, (uint32_t)TracyLine, color};
