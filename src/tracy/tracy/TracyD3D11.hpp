	...
	class D3D11Ctx {
		...
		public:
			...
			void Collect(CollectMode mode = POLL) {
				...
				D3D11_QUERY_DATA_TIMESTAMP_DISJOINT disjoint = {};
				if (m_immediateDevCtx
						->GetData(m_disjointQuery, &disjoint, sizeof(disjoint), D3D11_ASYNC_GETDATA_DONOTFLUSH)
					!= S_OK) {
					return;
				}
...
using TracyD3D11Ctx = tracy::D3D11Ctx*;
	...
	#define TracyD3D11SrcLocObject(name, color)                                                                        \
		static constexpr tracy::SourceLocationData                                                                     \
			TracyD3D11SrcLocSymbol {name, TracyFunction, TracyFile, (uint32_t)TracyLine, color};
