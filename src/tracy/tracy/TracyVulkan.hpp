...
namespace tracy {
	...
	class VkCtx {
		...
		public:
	...
	#endif
			{
				...
				if (VK_FUNCTION_WRAPPER(vkGetPhysicalDeviceCalibrateableTimeDomainsEXT)
					&& m_vkGetCalibratedTimestampsEXT) {
					FindAvailableTimeDomains(
						physdev,
						VK_FUNCTION_WRAPPER(vkGetPhysicalDeviceCalibrateableTimeDomainsEXT)
					);
				}
				...
				if (m_timeDomain == VK_TIME_DOMAIN_DEVICE_EXT) {
					...
					VK_FUNCTION_WRAPPER(vkGetQueryPoolResults(
						device,
						m_query,
						0,
						1,
	...
	class VkCtxScope {
		public:
			tracy_force_inline
			VkCtxScope(VkCtx* ctx, const SourceLocationData* srcloc, VkCommandBuffer cmdbuf, bool is_active)
	#ifdef TRACY_ON_DEMAND
				:
			...
			tracy_force_inline
			VkCtxScope(VkCtx* ctx, const SourceLocationData* srcloc, VkCommandBuffer cmdbuf, int depth, bool is_active)
	#ifdef TRACY_ON_DEMAND
				:
				m_active(is_active && GetProfiler().IsConnected())
	#else
	...
	#if defined TRACY_VK_USE_SYMBOL_TABLE
	static inline VkCtx* CreateVkContext(
		VkInstance instance,
		VkPhysicalDevice physdev,
		VkDevice device,
		VkQueue queue,
...
using TracyVkCtx = tracy::VkCtx*;
	...
	#define TracyVkContextName(ctx, name, size) ctx->Name(name, size);
		...
		#define TracyVkNamedZone(ctx, varname, cmdbuf, name, active)                                                   \
			...
			tracy::VkCtxScope                                                                                          \
				varname(ctx, &TracyConcat(__tracy_gpu_source_location, TracyLine), cmdbuf, TRACY_CALLSTACK, active);
