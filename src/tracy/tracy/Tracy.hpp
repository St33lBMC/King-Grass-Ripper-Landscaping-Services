...
#include "../common/TracySystem.hpp"
	...
	#define TracyNoop tracy::ProfilerAvailable()
		...
		#define ZoneTransient(varname, active)                                                                         \
			tracy::ScopedZone                                                                                          \
			varname(TracyLine, TracyFile, strlen(TracyFile), TracyFunction, strlen(TracyFunction), nullptr, 0, active)
		#define ZoneTransientN(varname, name, active)                                                                  \
			tracy::ScopedZone varname(                                                                                 \
				TracyLine,                                                                                             \
				TracyFile,                                                                                             \
		...
		tracy::Lockable<type> varname {                                                                                \
			[]() -> const tracy::SourceLocationData* {                                                                 \
				static constexpr tracy::SourceLocationData                                                             \
					srcloc {nullptr, #type " " #varname, TracyFile, TracyLine, 0};                                     \
				return &srcloc;                                                                                        \
			}()                                                                                                        \
		}
	...
	#define LockMark(varname)                                                                                          \
		static constexpr tracy::SourceLocationData                                                                     \
			__tracy_lock_location_##varname {nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0};               \
