...
namespace tracy {
	...
	bool SysTraceStart(int64_t& samplingPeriod) {
		...
		if (LookupPrivilegeValue(nullptr, SE_SYSTEM_PROFILE_NAME, &priv.Privileges[0].Luid) == 0)
			return false;

		HANDLE pt;
		if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &pt) == 0)
			return false;
		...
		if (isOs64Bit) {
			...
			if (intervalStatus != ERROR_SUCCESS)
				return false;
		...
		for (;;) {
			...
			if (ctxBufferIdx != numBuffers) {
				...
				if (activeNum > 0) {
					...
					for (int i = 0; i < ctxBufNum; i++) {
						if (end[i] != 0)
							ringArray[ctxBufferIdx + i].Advance(end[i]);
					}
				}
			}
			if (!traceActive.load(std::memory_order_relaxed))
