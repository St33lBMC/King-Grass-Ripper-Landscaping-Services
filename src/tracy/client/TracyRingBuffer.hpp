...
namespace tracy {
	...
	class RingBuffer {
		public:
			RingBuffer(unsigned int size, int fd, int id, int cpu = -1) : m_size(size), m_id(id), m_cpu(cpu), m_fd(fd) {
			...
			int64_t ConvertTimeToTsc(int64_t timestamp) const {
				if (!m_metadata->cap_user_time_zero)
					return 0;
			...
			uint64_t LoadHead() const {
				return std::atomic_load_explicit(
					(const volatile std::atomic<uint64_t>*)&m_metadata->data_head,
		...
		private:
			void StoreTail() {
				std::atomic_store_explicit(
					(volatile std::atomic<uint64_t>*)&m_metadata->data_tail,
					m_tail,
