...
void DBGHELP_UNLOCK;
	...
	#if TRACY_HAS_CALLSTACK == 2 || TRACY_HAS_CALLSTACK == 3 || TRACY_HAS_CALLSTACK == 4 || TRACY_HAS_CALLSTACK == 5   \
		|| TRACY_HAS_CALLSTACK == 6
...
extern "C" const char* ___tracy_demangle(const char* mangled) {
	if (!mangled || mangled[0] != '_')
		return nullptr;
	if (strlen(mangled) > ___tracy_demangle_buffer_len)
		return nullptr;
	int status;
...
namespace tracy {
	...
	class ImageCache {
		...
		private:
			...
			static int Callback(struct dl_phdr_info* info, size_t size, void* data) {
				...
				if (cache->Contains(startAddress))
					return 0;

				const uint32_t headerCount = info->dlpi_phnum;
				assert(headerCount > 0);
				const auto endAddress = reinterpret_cast<void*>(
					info->dlpi_addr + info->dlpi_phdr[info->dlpi_phnum - 1].p_vaddr
	...
	)(HANDLE hProcess, DWORD64 Address, ULONG InlineContext, PDWORD64 Displacement, PSYMBOL_INFO Symbol);
	...
	void DbgHelpInit() {
		...
		_SymGetLineFromInlineContext = (t_SymGetLineFromInlineContext
		)GetProcAddress(GetModuleHandleA("dbghelp.dll"), "SymGetLineFromInlineContext");
	...
	ModuleCache* LoadSymbolsForModuleAndCache(
	...
	void InitCallstack() {
		...
		if (initTimeModuleLoad && EnumDeviceDrivers(dev, sizeof(dev), &needed) != 0) {
			...
			s_krnlCacheCnt = cnt;
			std::sort(s_krnlCache, s_krnlCache + s_krnlCacheCnt, [](const KernelDriver& lhs, const KernelDriver& rhs) {
	...
	CallstackEntryData DecodeCallstackPtr(uint64_t ptr) {
		...
		if (_SymAddrIncludeInlineTrace) {
			...
			if (inlineNum > MaxCbTrace - 1)
				inlineNum = MaxCbTrace - 1;
			DWORD idx;
			if (inlineNum != 0)
				doInline = _SymQueryInlineTrace(proc, ptr, 0, ptr, ptr, &ctx, &idx);
		}
		if (doInline) {
			write = inlineNum;
			cb_num = 1 + inlineNum;
	...
	static void InitKernelSymbols() {
		...
		std::sort(tmpSym.begin(), tmpSym.end(), [](const KernelSymbol& lhs, const KernelSymbol& rhs) {
			return lhs.addr < rhs.addr;
		});
		for (size_t i = 0; i < tmpSym.size() - 1; i++) {
			if (tmpSym[i].name)
				tmpSym[i].size = tmpSym[i + 1].addr - tmpSym[i].addr;
		}
	...
	static int
	SymbolAddressDataCb(void* data, uintptr_t pc, uintptr_t lowaddr, const char* fn, int lineno, const char* function) {
	...
	static int
	CallstackDataCb(void* /*data*/, uintptr_t pc, uintptr_t lowaddr, const char* fn, int lineno, const char* function) {
	...
	CallstackEntryData DecodeCallstackPtr(uint64_t ptr) {
		...
		if (ptr >> 63 == 0) {
			...
			auto it = std::lower_bound(
				s_kernelSym,
				s_kernelSym + s_kernelSymCnt,
				ptr,
				[](const KernelSymbol& lhs, const uint64_t& rhs) { return lhs.addr + lhs.size < rhs; }
			);
