			...
			do {                                                                                                       \
				if (!(truth)) {                                                                                        \
					if (_memory_config.error_callback) {                                                               \
						_memory_config.error_callback(message " (" RPMALLOC_TOSTRING(truth                             \
						) ") at " __FILE__ ":" RPMALLOC_TOSTRING(__LINE__));                                           \
					} else {                                                                                           \
...
static FORCEINLINE void atomic_store32(atomic32_t* dst, int32_t val) {
	std::atomic_store_explicit(dst, val, std::memory_order_relaxed);
}

static FORCEINLINE int32_t atomic_incr32(atomic32_t* val) {
	return std::atomic_fetch_add_explicit(val, 1, std::memory_order_relaxed) + 1;
}
...
static FORCEINLINE int32_t atomic_add32(atomic32_t* val, int32_t add) {
	return std::atomic_fetch_add_explicit(val, add, std::memory_order_relaxed) + add;
}
...
static FORCEINLINE void atomic_store32_release(atomic32_t* dst, int32_t val) {
	std::atomic_store_explicit(dst, val, std::memory_order_release);
}

static FORCEINLINE int64_t atomic_load64(atomic64_t* val) {
	return std::atomic_load_explicit(val, std::memory_order_relaxed);
}
...
static FORCEINLINE void* atomic_exchange_ptr_acquire(atomicptr_t* dst, void* val) {
	return std::atomic_exchange_explicit(dst, val, std::memory_order_acquire);
}
...
namespace tracy {
	...
	static void* _rpmalloc_mmap_os(size_t size, size_t* offset) {
		...
		void* ptr = mmap(
			...
			(_memory_huge_pages ? MAP_HUGETLB : 0) | flags,
			-1,
			0
		);
		...
		void* ptr = mmap(
			...
			(_memory_huge_pages ? MAP_ALIGNED(align) : 0) | flags,
			-1,
			0
		);
		...
		void* ptr =
			mmap(base, size + padding, PROT_READ | PROT_WRITE, (_memory_huge_pages ? MAP_ALIGN : 0) | flags, -1, 0);
		#else
		...
		rpmalloc_assert(
			(size < _memory_span_size) || !((uintptr_t)ptr & ~_memory_span_mask),
	...
	static void _rpmalloc_span_mark_as_subspan_unless_master(span_t* master, span_t* subspan, size_t span_count) {
		rpmalloc_assert(
			(subspan != master) || (subspan->flags & SPAN_FLAG_MASTER),
	...
	static void _rpmalloc_span_unmap(span_t* span) {
		...
		if (atomic_add32(&master->remaining_spans, -(int32_t)span_count) <= 0) {
			...
			rpmalloc_assert(
				!!(master->flags & SPAN_FLAG_MASTER) && !!(master->flags & SPAN_FLAG_SUBSPAN),
				"Span flag corrupted"
			);
	...
	static uint32_t free_list_partial_init(
		void** list,
		void** first_block,
		void* page_start,
		void* block_start,
	...
	static span_t* _rpmalloc_heap_extract_new_span(
		heap_t* heap,
	...
	static heap_t* _rpmalloc_heap_allocate_new(void) {
		...
		if (span_count > heap_span_count) {
			...
			size_t reserve_count =
				(remain_count > _memory_heap_reserve_count ? _memory_heap_reserve_count : remain_count);
	...
	_rpmalloc_allocate_from_heap_fallback(heap_t* heap, heap_size_class_t* heap_size_class, uint32_t class_idx) {
		...
		if (EXPECTED(span != 0)) {
			rpmalloc_assert(
				span->block_count == _memory_size_class[span->size_class].block_count,
	...
	static void _rpmalloc_deallocate_direct_small_or_medium(span_t* span, void* block) {
		...
		rpmalloc_assert(
			heap->owner_thread == get_thread_id() || !heap->owner_thread || heap->finalize,
			"Internal failure"
		);
	...
	#if RPMALLOC_FIRST_CLASS_HEAPS
		int defer =
			(span->heap->owner_thread && (span->heap->owner_thread != get_thread_id()) && !span->heap->finalize);
	#else
	...
	static void _rpmalloc_deallocate_large(span_t* span) {
		...
		const int set_as_reserved =
			((span->span_count > 1) && (heap->span_cache.count == 0) && !heap->finalize && !heap->spans_reserved);
	#else
		...
		if (set_as_reserved) {
			...
			if (span->flags & SPAN_FLAG_MASTER) {
				...
				rpmalloc_assert(master->flags & SPAN_FLAG_MASTER, "Span flag corrupted");
				rpmalloc_assert(
					atomic_load32(&master->remaining_spans) >= (int32_t)span->span_count,
	...
	static void* _rpmalloc_aligned_reallocate(
		heap_t* heap,
		void* ptr,
		size_t alignment,
		size_t size,
	...
	TRACY_API int rpmalloc_initialize(void) {
	...
	int rpmalloc_initialize_config(const rpmalloc_config_t* config) {
		...
		_memory_heap_reserve_count =
			(_memory_span_map_count > DEFAULT_SPAN_MAP_COUNT) ? DEFAULT_SPAN_MAP_COUNT : _memory_span_map_count;
	...
	TRACY_API void rpmalloc_finalize(void) {
		rpmalloc_thread_finalize(1);
	...
	TRACY_API void rpmalloc_thread_initialize(void) {
	...
	TRACY_API void rpmalloc_thread_finalize(int release_caches) {
		heap_t* heap = get_thread_heap_raw();
		if (heap)
	...
	TRACY_API RPMALLOC_ALLOCATOR void* rpmalloc(size_t size) {
	#if ENABLE_VALIDATE_ARGS
	...
	TRACY_API void rpfree(void* ptr) {
		_rpmalloc_deallocate(ptr);
	}
	...
	TRACY_API RPMALLOC_ALLOCATOR void* rprealloc(void* ptr, size_t size) {
	#if ENABLE_VALIDATE_ARGS
		if (size >= MAX_ALLOC_SIZE) {
			errno = EINVAL;
			return ptr;
	...
	extern inline RPMALLOC_ALLOCATOR void* rpmalloc_heap_alloc(rpmalloc_heap_t* heap, size_t size) {
		#if ENABLE_VALIDATE_ARGS
		if (size >= MAX_ALLOC_SIZE) {
			errno = EINVAL;
			return 0;
	...
	extern inline RPMALLOC_ALLOCATOR void* rpmalloc_heap_calloc(rpmalloc_heap_t* heap, size_t num, size_t size) {
