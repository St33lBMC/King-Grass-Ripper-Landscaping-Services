...
extern "C" typedef BOOL(WINAPI* t_GetLogicalProcessorInformationEx
...
*/
	...
	static std::vector<MappingInfo> ParseMappings() {
		...
		while (fgets(line, sizeof(line), file)) {
			...
			if (sscanf(line, "%lx-%lx", &start_addr, &end_addr) != 2)
				continue;
		#else
			if (sscanf(line, "%dx-%dx", &start_addr, &end_addr) != 2)
				continue;
		#endif
			...
			if (!second_space || second_space - perm != 4)
				continue;
			result.emplace_back();
	...
	static bool EnsureReadable(MappingInfo& mapping) {
		...
		if (mapping.perm_x)
			prot |= PROT_EXEC;
		if (mprotect(reinterpret_cast<void*>(mapping.start_address), mapping.end_address - mapping.start_address, prot)
			== -1) {
	...
	static const char* GetHostInfo() {
		...
		ptr +=
			sprintf(ptr, "Compiler: MSVC clang-cl %i.%i.%i\n", __clang_major__, __clang_minor__, __clang_patchlevel__);
		#else
			...
			case EXCEPTION_ACCESS_VIOLATION:
				...
				switch (pExp->ExceptionRecord->ExceptionInformation[0]) {
					case 0:
						msgPtr += sprintf(
							msgPtr,
							"Read violation at address 0x%" PRIxPTR ".",
					...
					case 1:
						msgPtr += sprintf(
							msgPtr,
							"Write violation at address 0x%" PRIxPTR ".",
							pExp->ExceptionRecord->ExceptionInformation[1]
						);
						break;
					case 8:
						msgPtr += sprintf(
							msgPtr,
							"DEP violation at address 0x%" PRIxPTR ".",
							pExp->ExceptionRecord->ExceptionInformation[1]
						);
						break;
					default:
	...
	static void CrashHandler(int signal, siginfo_t* info, void* /*ucontext*/) {
		bool expected = false;
		if (!s_alreadyCrashed.compare_exchange_strong(expected, true))
			ThreadFreezer(signal);
	...
	static ProfilerData& GetProfilerData() {
		...
		if (!ptr) {
			int expected = 0;
			while (!profilerDataLock
						.compare_exchange_weak(expected, 1, std::memory_order_release, std::memory_order_relaxed)) {
				expected = 0;
				YieldThread();
			}
	...
	TRACY_API moodycamel::ConcurrentQueue<QueueItem>& GetQueue() {
		return GetProfilerData().queue;
	}

	TRACY_API int64_t GetInitTime() {
		return GetProfilerData().initTime;
	}

	TRACY_API std::atomic<uint32_t>& GetLockCounter() {
		return GetProfilerData().lockCounter;
	}

	TRACY_API std::atomic<uint8_t>& GetGpuCtxCounter() {
		return GetProfilerData().gpuCtxCounter;
	}

	TRACY_API GpuCtxWrapper& GetGpuCtx() {
		return GetProfilerThreadData().gpuCtx;
	}
	...
	std::atomic<ThreadNameData*>& GetThreadNameData() {
		return GetProfilerData().threadNameData;
	}

		#ifdef TRACY_ON_DEMAND
	TRACY_API LuaZoneState& GetLuaZoneState() {
		return GetProfilerThreadData().luaZoneState;
	}
		#endif
	...
	TRACY_API int64_t GetInitTime() {
		return s_initTime.val;
	}

	TRACY_API std::atomic<uint32_t>& GetLockCounter() {
		return s_lockCounter;
	}

	TRACY_API std::atomic<uint8_t>& GetGpuCtxCounter() {
		return s_gpuCtxCounter;
	}
	...
	Profiler::DequeueStatus Profiler::Dequeue(moodycamel::ConsumerToken& token) {
		...
		const auto sz = GetQueue().try_dequeue_bulk_single(
			...
			[this, &connectionLost](const uint32_t& threadId) {
				if (ThreadCtxCheck(threadId) == ThreadCtxStatus::ConnectionLost)
					connectionLost = true;
			},
				...
				while (sz-- > 0) {
					...
					if (idx < (int)QueueType::Terminate) {
						switch ((QueueType)idx) {
							...
							case QueueType::SymbolInformation: {
								auto fileString =
									(const char*)MemRead<uint64_t>(&item->symbolInformationFat.fileString);
							...
							case QueueType::ExternalNameMetadata: {
								...
								auto threadName =
									(const char*)MemRead<uint64_t>(&item->externalNameMetadata.threadName);
	...
	Profiler::DequeueStatus
	Profiler::DequeueContextSwitches(tracy::moodycamel::ConsumerToken& token, int64_t& timeStop) {
		const auto sz = GetQueue().try_dequeue_bulk_single(
			token,
			[](const uint64_t&) {},
	...
	Profiler::ThreadCtxStatus Profiler::ThreadCtxCheck(uint32_t threadId) {
		...
		if (!AppendData(&item, QueueDataSize[(int)QueueType::ThreadContext]))
			return ThreadCtxStatus::ConnectionLost;
	...
	bool Profiler::CommitData() {
		...
		if (m_bufferOffset > TargetFrameSize * 2)
			m_bufferOffset = 0;
	...
	void Profiler::SendString(uint64_t str, const char* ptr, size_t len, QueueType type) {
		assert(
			type == QueueType::StringData || type == QueueType::ThreadName || type == QueueType::PlotName
	...
	void Profiler::CalibrateDelay() {
		...
		static const tracy::SourceLocationData
			__tracy_source_location {nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0};
	...
	void Profiler::ReportTopology() {
		...
		t_GetLogicalProcessorInformationEx _GetLogicalProcessorInformationEx = &::GetLogicalProcessorInformationEx;
			#else
		t_GetLogicalProcessorInformationEx _GetLogicalProcessorInformationEx = (t_GetLogicalProcessorInformationEx
...
___tracy_emit_zone_begin_callstack(const struct ___tracy_source_location_data* srcloc, int depth, int active) {
	___tracy_c_zone_context ctx;
	#ifdef TRACY_ON_DEMAND
	ctx.active = active && tracy::GetProfiler().IsConnected();
	#else
...
TRACY_API void
___tracy_emit_memory_alloc_callstack_named(const void* ptr, size_t size, int depth, int secure, const char* name) {
...
TRACY_API uint64_t ___tracy_alloc_srcloc(
	uint32_t line,
	const char* source,
	size_t sourceSz,
	const char* function,
...
TRACY_API uint64_t ___tracy_alloc_srcloc_name(
	uint32_t line,
	const char* source,
	size_t sourceSz,
	const char* function,
...
TRACY_API void ___tracy_mark_lockable_ctx(
	struct __tracy_lockable_context_data* lockdata,
...
TRACY_API void
___tracy_custom_name_lockable_ctx(struct __tracy_lockable_context_data* lockdata, const char* name, size_t nameSz) {
...
TRACY_API void ___tracy_fiber_enter(const char* fiber) {
	tracy::Profiler::EnterFiber(fiber, 0);
}
